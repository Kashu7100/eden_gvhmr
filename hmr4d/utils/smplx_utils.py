...
def make_smplx(type="neu_fullpose", **kwargs):
    if type == "neu_fullpose":
        ...
        model = BodyModelSMPLX(model_path=get_checkpoint_root() / "body_models", **bm_kwargs)
        ...
        model = BodyModelSMPLH(model_path=get_checkpoint_root() / "body_models", **bm_kwargs)

    elif type in ["smplx-circle", "smplx-groundlink"]:
        # don't use hand
        bm_kwargs = {
            "model_path": get_checkpoint_root() / "body_models",
            "model_type": "smplx",
        ...
        bm_kwargs = {
            "model_path": get_checkpoint_root() / "body_models/smplx",
            "gender": kwargs.get("gender"),
            "num_betas": 10,
            "num_expression": 10,
        }
